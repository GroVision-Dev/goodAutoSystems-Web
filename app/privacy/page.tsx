...
const SECTIONS = [
  {
    ...
    body: "회사는 회원가입 및 서비스 제공을 위해 다음의 개인정보를 수집합니다. 필수항목: 아이디, 이름, 이메일, 휴대폰 번호, 비밀번호(암호화 저장). 결제 시: 결제 정보는 결제대행사 포트원(PortOne)과 연동 PG사가 처리하며 회사는 카드번호 등 민감한 결제 정보를 저장하지 않습니다. 서비스 이용 과정에서 자동 생성되는 정보: 접속 일시, 조회한 페이지, 브라우저 정보, 일부가 가려진(마스킹된) IP 주소, 방문자 구분용 쿠키 식별값이 서비스 이용 통계 목적으로 수집됩니다.",
  },
  {
