...
async function main() {
  ...
  // 가격 변경 (2026-09-11): 기존 DB가 아직 이전 가격이면 1회 갱신한다. 관리자가 바꾼 값은 유지.
  const PRICE_UPDATES = [
    { slug: "goodauto-pro", from: 99000, to: 300000 },
    { slug: "ai-automation-starter", from: 490000, to: 990000 },
    { slug: "ai-automation-enterprise", from: 1900000, to: 2500000 },
  ];
  for (const update of PRICE_UPDATES) {
    const result = await prisma.product.updateMany({
      where: { slug: update.slug, price: update.from },
      data: { price: update.to },
    });
    if (result.count > 0) {
      console.log(`${update.slug} 가격 ${update.from.toLocaleString()} → ${update.to.toLocaleString()}원`);
    }
  }
